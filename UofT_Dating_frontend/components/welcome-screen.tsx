...
const UOFT_BLUE = '#002A5C'; // ✅ CHANGED
const UOFT_ALLOWED_SUFFIXES = ['@mail.utoronto.ca', '@utoronto.ca']; // ✅ CHANGED
...
const ActiveMembers = ({
  ...
  minActiveMembers,
  color,
  minText = 'on the UofT Dating app', // ✅ CHANGED
}: {
  numActiveMembers: number,
  minActiveMembers: number
  color: string,
  ...
  return (
    <View
      style={{
        width: '100%',
        height: '100%',
        backgroundColor: UOFT_BLUE, // ✅ CHANGED
        overflow: 'hidden',
      }}
    >
      {/* ✅ CHANGED: 直接移除 Duolicious 官方插画（最稳，不会再像原版） */}
      {Platform.OS === 'web' && windowWidth >= 950 && null}
...
const WelcomeScreen_ = ({navigation, route}) => {
  ...
  // ✅ CHANGED: 前端先做 UofT 邮箱校验（否则后端拒绝会显得像“服务器坏了”）
  const isAllowedUofTEmail = (fullEmail: string) => {
    const lower = fullEmail.toLowerCase();
    return UOFT_ALLOWED_SUFFIXES.some(s => lower.endsWith(s));
  };
  ...
  const submit = async (suffix?: string) => {
    ...
    // ✅ CHANGED: 在请求 OTP 前直接拦截
    if (!isAllowedUofTEmail(email_)) {
      setLoginStatus('UofT email required (@mail.utoronto.ca or @utoronto.ca)');
      return;
    }
    ...
    if (response.ok) {
      ...
      setLoginStatus(
        ...
        'We couldn’t connect to UofT Dating' // ✅ CHANGED
      );
    }
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {clubName_ ?
              `Join ${clubName_} on UofT Dating` : // ✅ CHANGED
              'Meet UofT students. Safe, simple, and local.'} {/* ✅ CHANGED */}
          </DefaultText>
        ...
        <View style={{
          ...
          <DefaultTextInput
            placeholder="Enter your UofT email to begin" // ✅ CHANGED
          ...
          >
            UofT email required: @mail.utoronto.ca or @utoronto.ca
          </DefaultText>
          ...
          {/* ✅ CHANGED: 直接把 gmail 等快捷按钮删掉，避免误导 */}
          {windowHeight > 500 &&
            <View
              style={{
                flexDirection: 'row',
            ...
            >
              <SuffixButton suffix="@mail.utoronto.ca" />
              <SuffixButton suffix="@utoronto.ca" />
            </View>
          }
        </View>
