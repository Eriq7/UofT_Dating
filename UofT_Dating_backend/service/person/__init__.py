...
def _is_ip_blocked_by_firehol(ip: str | None) -> bool:
    """
    DEV/DEMO stability:
    FireHOL worker can crash (e.g., EOFError) under Docker Desktop, causing OTP endpoints to 500.
    If FireHOL fails, treat as NOT blocked so local signup flow can proceed.
    """
    if not ip:
        return False
    try:
        return firehol.matches(ip)
    except Exception:
        return False
...
def post_request_otp(req: t.PostRequestOtp):
    ...
    # ✅ UofT 邮箱：跳过 FireHOL + Disposable 检查（只改这一段）
    if not (email.endswith('@utoronto.ca') or email.endswith('@mail.utoronto.ca')):
        if _is_ip_blocked_by_firehol(request.remote_addr):
