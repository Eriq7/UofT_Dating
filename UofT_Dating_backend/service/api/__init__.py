...
# ============================================================
# IMPORTANT DEV PATCH:
# - Disable rate limit on /request-otp to avoid 429 ("You're doing that too much")
# - This is the fastest, most stable way to unblock local testing.
# ============================================================

@post('/request-otp', limiter=limiter.exempt)   # <-- changed: was shared_otp_limit
@validate(t.PostRequestOtp)
def post_request_otp(req: t.PostRequestOtp):
    # Original rate-limit logic (kept for reference):
    #
    # limit = "40 per day"
    # scope = "request_otp"
    # with (
